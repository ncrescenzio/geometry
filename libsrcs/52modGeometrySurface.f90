...
module GeometrySurface
  ...
  type, extends(abs_simplex_mesh), public :: mesh
     ...
     !integer :: nnode_bc = 0
     !> Kind of surface
     !> default = 0 (flat surface)
     integer :: flag_bottom = 0
     ...
     !> edge_plist, side_cnc, iside, neigh, peri_cell,length_edge, bar_edge, normal
     ...
     !> Nodal coordinate. 
     !> **Input in the constructor.**
     !> Pointer to the Nodal coordinate. 
     !> **Input in the constructor.**
     real(kind=double), pointer :: NodCoord(:,:)
     !>-------------------------------------------------------------
     ...
     !> Dimension (2,nedge).
     !> Area Left and Right of the 'edge diamond'
     !> **Computed in procedure Prop.**
     real(kind=double), allocatable :: area_edge(:,:)
     !> Dimension (3,nedge).
     !> normal vector to the edge (consistent with iside and edge_plist)
     !> **Computed in procedure Prop.**
     real(kind=double), allocatable :: normal_edge(:,:)
     !> Dimension (3,nedge). 
   ...
   contains
     ...
     procedure, public, pass :: refine_surf => build_refined_triang_coord
     ...
     !> Calculate cell areas, edge lengths, normals, etc
     !> (private procedure for type mesh, used in init)
     procedure, public, pass :: meshProp
     !> calculate Lebesgue p-norm of piecewise-constant array on cells
     !> (public procedure for type mesh)
     procedure, public, pass :: normp_cell
     !> calculate Lebesgue p-norm of piecewise-constant values on edges
     !> (public procedure for type mesh)
     procedure, public, pass :: normp_edge
...
contains
  ...
  subroutine init_mesh(this,lun_err,&
       ...
       flag_cnc, flag_reorder, flag_bottom, time)
    use Globals
    implicit none
    !vars
    ...
    integer,     optional, intent(in   ) :: flag_cnc,flag_reorder,flag_bottom
    real(kind=double), optional, intent(in) :: time
    !local 
    logical :: rc
    integer :: res
    integer, allocatable :: perm(:), inv_perm(:)
    real(kind=double) :: time_loc
    ...
    this%cell_id = 5
    if(present(flag_bottom)) this%flag_bottom=flag_bottom
    time_loc = zero
    ...
    case (1)
       ...
       call this%refine_surf(lun_err,input_mesh,time_loc)
    case (2)
    ...
    ! Assign pointer coord
    this%NodCoord => this%coord
  ...
  !>
  !<-------------------------------------------------------------
  subroutine build_refined_triang_coord(this, lun_err,input_mesh,time)
    use Globals
    implicit none
    ! External variables
    class(mesh), intent(inout) :: this
    ...
    this%flag_bottom = local_mesh%flag_bottom
    ...
    do iedge=1,local_mesh%nedge
       this%coord(1:2,local_mesh%nnode+iedge)=local_mesh%bar_edge(1:2,iedge)
       x3 = x3value(local_mesh%flag_bottom,&
            local_mesh%bar_edge(1,iedge),local_mesh%bar_edge(2,iedge),&
            time_loc)
       this%coord(3,local_mesh%nnode+iedge) = x3
    end do
    ...
    this%normal_storage = 'cell'
    allocate(this%normal_cell(3,this%ncell),stat=res)
    if(res .ne. 0) rc = IOerr(lun_err, err_alloc, 'refine_mesh_surf', &
         '  type mesh member normal_node ',res)
    do icell=1,this%ncell
       n1 = this%topol(1,icell)
       n2 = this%topol(2,icell)
  ...
  contains
     !>-------------------------------------------------------------
    !> Procedure that calculates the x3value
    !> from the analytic function for the bottom
    !> (private procedure)
    !>
    !> usage:
    !>     res = x3value(flag_bottom,s1,s2,time)
    !>
    !> where:
    !> \param[in] flag_bottom -> integer. flag=0 FLAT CASE (default)
    !>                                    flag=1 PARABOLA CASE
    !>                                    flag=2 HYPERBOLOID CENTRAL BUMP
    !>                                    flag=3 FULLY 3D SURFACE
    !> \param[in] s1,s2 -> real(double). s1, s2 coordinates
    !> \param[out] res -> real(double). x3 value at the point (s1,s2)
    !>
    !<-------------------------------------------------------------
    function  x3value(flag_bottom,s1,s2,time) result(res)
      implicit none
      ! vars
      integer, intent(in) :: flag_bottom
      real(kind=double), intent(in) :: s1,s2,time
      real(kind=double) :: res
      ! local vars

      select case(flag_bottom)
      case(1)
         ! CASO piano -0.10*x+1
         res = -0.1*s1 + one
      CASE(11)
         ! CASO DOPPIO_PIANO
         if (s1.le.5.0d0) then
            res = -s1/three + 13.0d0/6.0d0
         else
            res = -0.1*s1 + one
         end if
      case(12)
         ! CASO piano (che trasla) - z = 0.4*(10-x)+0.1*time
         res = -0.4*(s1-10) + 0.1*time
      case(2)
         ! CASO Parabola_Soft - z = 0.04*(10-x)(10-tx)
         res = 0.04 * (s1-10)*(10-time*s1)
      case(3)
         ! CASO HYPERBOLOID_BUMP_2
         res = -0.8*sqrt(s1**2+s2**2+1)+one 
      CASE(4)
         ! CASO SURFACE3D_casoB -> -0.002*(x**3) - 0.01* x *(y**2)
         res = -0.002*(s1**3) - 0.01* s1 *(s2**2)
      CASE(5)
         ! CASO UPPER EMISPHERE -> sqrt(1-x**2-y**2)
         res = sqrt(one-s1**2-s2**2)
      CASE(6)
         ! CASO LOWER EMISPHERE -> -sqrt(1-x**2-y**2)+1
         RES = -sqrt(one-S1**2-S2**2) + one
  ...
  subroutine kill_mesh(this, lun)
    ...
    case (1)
       ...
       if ( allocated(this%peri_cell) ) then
          deallocate(this%peri_cell,stat=res)
          if (res.ne.0) rc=IOerr(lun, err_dealloc, 'kill_mesh', &
               'mesh members peri_cell')
       end if
       
       if ( allocated(this%length_edge) ) then
          deallocate(this%length_edge,stat=res)
          if (res.ne.0) rc=IOerr(lun, err_dealloc, 'kill_mesh', &
               'mesh members length_edge')
       end if

       if ( allocated(this%area_edge) ) then
          deallocate(this%area_edge,stat=res)
          if (res.ne.0) rc=IOerr(lun, err_dealloc, 'kill_mesh', &
               'mesh members area_edge')
       end if

       if ( allocated(this%normal_edge) ) then
          deallocate(this%normal_edge,stat=res)
          if (res.ne.0) rc=IOerr(lun, err_dealloc, 'kill_mesh', &
               'mesh members normal_edge')
       end if
    ...
    case (2)
       ...
       deallocate(this%peri_cell,this%length_edge,this%area_edge, &
            this%normal_edge,this%bar_edge,stat=res)
       if (res.ne.0) rc=IOerr(lun, err_dealloc, 'kill_mesh', &
            'mesh members peri_cell, length_edge, area_edge, normal_edge, bar_edge')
  ...
  subroutine info_mesh(this, lun)
    ...
    if (this%cnc_built.ge.1) then
       ...
       write(lun,*) ' '    
       write(lun,*) 'Edge Area: (type mesh member area_edge)'
       do j=1,this%nedge
          write(lun,'(i8,2x,2(1pe12.4))') j,this%area_edge(1,j), this%area_edge(2,j)
       end do
       write(lun,'(a,(1pe12.4))') 'Domain area =',&
            sum(this%area_edge(1,:))+sum(this%area_edge(2,:))

!!$       write(lun,*) ' '    
!!$       write(lun,*) 'components of edge normals: (type mesh member normal_edge)'
!!$       write(lun,*) '   edge    n(1)        n(2)'
!!$       do j=1,this%nedge
!!$          write(lun,'(i8,2x,2(1pe12.4))') j,this%normal_edge(1,j),this%normal_edge(2,j)
!!$       end do
    end if
  ...
  subroutine normal_projection(this, vec, proj)
    ...
    do iedge=1,this%nedge
       locvec = vec(:,iedge)
       locnorm = this%normal_cell(:,iedge)
  ...
  subroutine connection(this, lun)
    ...
    do iedg=1,nedge
       ...
       this%plist(:,iedg)=edge_plist(:,perm(iedg))       
    end do
    do icell=1,ncell
       do iloc=1,3         
  ...
  subroutine meshProp(this, lun)
    ...
    if (this%cnc_built .ge. 1) then
       ...
       if (.not. allocated(this%area_edge)) then
          allocate(this%area_edge(2,this%nedge),stat=res)
          if(res .ne. 0) rc = IOerr(lun, err_alloc, 'meshProp', &
               '  type mesh member area_edge (array)',res)
          this%area_edge = zero
       end if
    ...
    if (this%cnc_built .ge. 1) then
       ...
       do iedg=1,this%nedge
          ...
          ! Area Edge
          celL = this%edge_plist(1,iedg)
          celR = this%edge_plist(2,iedg)
          if(celL.ne.0) &
               this%area_edge(1,iedg) = this%size_cell(celL)*onethird
          if(celR.ne.0) &
               this%area_edge(2,iedg) = this%size_cell(celR)*onethird
       end do
...
!>------------------------------------------------------------------
  ...
  !>------------------------------------------------------------------
  !> Evaluates the p-norm of functions piecewise constant on cell
  !>------------------------------------------------------------------
  ...
  !>------------------------------------------------------------------
  !> Evaluates the p-norm of functions piecewise constant on edges
  !>------------------------------------------------------------------
  function normp_edge(this,power,data) result(total)
    use Globals
    implicit none
    class(mesh),       intent(in) :: this
    real(kind=double), intent(in) :: power
    real(kind=double), intent(in) :: data(this%nedge)
    real(kind=double) :: total
    !local 
    integer :: iedge
    real(kind=double) :: ddot
    real(kind=double) :: Aedge(this%nedge)

    Aedge = this%area_edge(1,:)+this%area_edge(2,:)

    if ( power .eq.  0 ) then
       total = maxval( abs( data ) )
    else if ( power .eq. 1 ) then
       total = ddot(this%nedge, abs(data) ,1,Aedge,1)
    else
       total = zero
       do iedge = 1, this%nedge
          total = total + abs(data(iedge)) ** power * Aedge(iedge)
       end do
       total = total ** ( one / power )
    end if
  end function normp_edge
  ...
  function meshpar(this,flag) result(h)
    ...
    if (flag .eq. 1) h=minval(sqrt(this%size_cell*two))
    if (flag .eq. 2) h=maxval(sqrt(this%size_cell*two))
